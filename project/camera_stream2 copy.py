        ...
        try:
            while True:
                ...
                if ids is not None and len(ids) > 0:
                    ...
                    valid_mask = distances <= 3.0 # 3m 밖의 마커는 인식하지 않도록 함 (localization 안정성) 
                    ...
                    # ==== Multi-marker Fusion 준비 ====
                    cam_positions = []
                    cam_angles = []
                    R_list = []

                    for mid, rvec, tvec, corner in zip(ids, rvecs, tvecs, corners):
                        mid = int(mid[0])
                        ...
                        pos_i = (-R.T @ tvec_g).flatten()
                        ang_i = np.degrees(self._rotation_matrix_to_euler_angles(R.T))

                        cam_positions.append(pos_i)
                        cam_angles.append(ang_i)
                        R_list.append(R)
                    ...
                    if len(cam_positions) > 0:
                        ...
                        cam_angles = np.array(cam_angles)

                        # 거리 기반 가중치 (멀리 있는 마커 영향 작게)
                        distances = np.linalg.norm(cam_positions, axis=1)
                        weights = 1 / (distances**2 + 1e-6)
                        weights /= np.sum(weights)

                        # 가중 평균
                        pos = np.sum(cam_positions * weights[:, None], axis=0)
                        ang = np.sum(cam_angles * weights[:, None], axis=0)

                        # ==== smoothing (position + angle) ====
                        if not hasattr(self, 'filtered_pos'):
                            self.filtered_pos = pos
                        else:
                            alpha = 0.8
                            self.filtered_pos = alpha*self.filtered_pos + (1-alpha)*pos

                        if not hasattr(self, 'filtered_ang'):
                            self.filtered_ang = ang
                        else:
                            alpha = 0.7
                            self.filtered_ang = alpha*self.filtered_ang + (1-alpha)*ang

                        stable_pos = self.filtered_pos
                        stable_ang = self.filtered_ang

                        # publish
                        coord_str = f"{stable_pos[0]},{stable_pos[1]},{stable_pos[2]},{stable_ang[0]},{stable_ang[1]},{stable_ang[2]}"
                        self.sock.sendto(coord_str.encode('utf-8'), self.server_address)
                        self.current_position = stable_pos

                        # 화면 표시
                        self._put_localization_text(color_image, stable_pos, stable_ang)
